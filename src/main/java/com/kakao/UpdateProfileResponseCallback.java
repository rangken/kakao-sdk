...
/**
 * 사용자정보 저장 요청 ({@link UserManagement#requestUpdateProfile(UpdateProfileResponseCallback, java.util.Map)}) 호출할 때 넘겨주고 콜백을 받는다.
 * @author MJ
 */
public abstract class UpdateProfileResponseCallback extends UserResponseCallback {
    /**
     * 사용자정보 저장을 성공적으로 마친 경우로
     * 일반적으로 view를 update하도록 구현한다.
     * @param userId 정보 저장이 완료된 사용자의 id
     */
    protected abstract void onSuccess(final long userId);

    /**
     * 사용자정보 저장 요청전이나 요청 결과가 세션이 close된 경우로
    ...
    /**
     * 세션이 닫힌 경우({@link #onSessionClosedFailure})를 제외한 이유로 사용자정보 저장이 실패한 경우
     ...
     * {@link ErrorCode#NOT_REGISTERED_PROPERTY_KEY_CODE}, <br/>
    ...
    /**
     * UserResponseCallback를 구현한 것으로 사용자정보 저장 요청이 성공했을 때 호출된다.
     * 사용자 콜백 {@link #onSuccess(long)}을 호출한다.
     * 결과 User 객체가 비정상이면 에러처리한다.
     * @param user 정보 저장된 사용자 id를 포함한 사용자 객체
     */
    @Override
    protected void onSuccessUser(final User user) {
        if(user == null || user.getId() <= 0)
            onError("UpdateProfileResponseCallback : onSuccessUser is called but the result user is null.", new APIErrorResult(null, "the result of updateProfile request is null."));
        else
            onSuccess(user.getId());
    }

    /**
     * {@link com.kakao.http.HttpResponseHandler}를 구현한 것으로 사용자정보 저장 요청 전 또는 요청 중에 세션이 닫혀 로그인이 필요할 때 호출된다.
    ...
    protected void onHttpSessionClosedFailure(final APIErrorResult errorResult) {
        Logger.getInstance().d("UpdateProfileResponseCallback : session is closed before requesting updateProfile. errorResult = " + errorResult);
        onSessionClosedFailure(errorResult);
    }

    /**
     * {@link com.kakao.http.HttpResponseHandler}를 구현한 것으로 사용자정보 저장 요청 중에 서버에서 요청을 수행하지 못했다는 결과를 받았을 때 호출된다.
    ...
    protected void onHttpFailure(final APIErrorResult errorResult) {
        onError("UpdateProfileResponseCallback : server error occurred during requesting updateProfile.", errorResult);
    }
