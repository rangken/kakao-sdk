...
public class Session {
    ...
    public void open(final SessionCallback sessionCallback) {
        internalOpen(sessionCallback, AuthType.KAKAO_TALK);
    }

    public void open(final SessionCallback sessionCallback, final AuthType authType) {
        internalOpen(sessionCallback, authType);
    }
    ...
    private Session(final Context context){
        ...
        if(appKey == null)
            throw new KakaoException(ERROR_TYPE.MISS_CONFIGURATION, String.format("need to declare %s in your AndroidManifest.xml", CommonProtocol.APP_KEY_PROPERTY));
    ...
    private void internalOpen(final SessionCallback sessionCallback, final AuthType authType) {
        // 이미 open이 되어 있다.
                ...
                switch (state) {
                    case CLOSED:
                        if (appKey != null && redirectUri != null) {
                            ...
                            requestLogin(getLoginActivityIntent(authorizationCodeRequest, authType));
                        } else {
    ...
    private Intent getLoginActivityIntent(final AuthorizationCodeRequest authCodeRequest) {
        return getLoginActivityIntent(authCodeRequest, AuthType.KAKAO_TALK);
    }

    private Intent getLoginActivityIntent(final AuthorizationCodeRequest authCodeRequest, final AuthType authType) {
        final Intent intent = new Intent(context, LoginActivity.class);
        intent.putExtra(LoginActivity.CODE_REQUEST_KEY, authCodeRequest);
        intent.putExtra(LoginActivity.CODE_REQUEST_TYPE_KEY, authType.getNumber());
        return intent;
    }
